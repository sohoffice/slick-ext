    ...
    override def getValue(r: ResultSet, idx: Int): Point = {
      r.getObject(idx) match {
        ...
        case null =>
          null
        case g: PGgeometry =>
          throw new SlickException(s"Database geometry data is not a point. geoType: ${g.getGeoType}")
        case o: PGobject =>
          throw new SlickException(s"Database geometry data is a PGobject. type: ${o.getType}")
        case x =>
          throw new SlickException(s"Database geometry data is not a PGgeometry. class: $x @ ${x.getClass}")
      }
    }
